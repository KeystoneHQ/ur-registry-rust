...
use secp256k1::{XOnlyPublicKey, Parity};
use std::ffi::{CStr};
...
#[no_mangle]
pub extern "C" fn crypto_hd_key_get_uncompressed_key_data(compressed_key: PtrString) -> PtrResponse {
    ...
    let parity = if prefix.eq("02") { Parity::Even } else { Parity::Odd };
